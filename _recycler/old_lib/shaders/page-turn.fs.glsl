...
vec4 turnPage(vec2 fragCoord)
{
    ...
    if (iMouse.x==0. && iMouse.y==0. && anim)
    {
        firstcycle = mod(iGlobalTime/3.5, 2.)<1.;
        mpoint = vec2(mod(iGlobalTime/3.5, 1.)*iResolution.x*2.0, pow(mod(iGlobalTime/3.5, 1.), 2.5)*iResolution.y/1.2 + 8.*smoothstep(0., 0.07, mod(iGlobalTime/3.5, 1.)));
    }
    else
    {
        mpoint = iMouse.xy;
    ...
    vec2 uvr = rotateVec(uv - midmpoint/iResolution.yy, angle);
    ...
    if (uvr.x>0. && uvr3b.y>0.)
    {
        ...
        if(uvr3.x>=0. || uvr3.y<=0.)
        {
        	...
        	vec2 uvrs = rotateVec(uv - midmpoint/iResolution.yy - shadowoffset, angle);
            ...
            if (firstcycle)
                ...
                i = difft*(colors?vec4(1., 0.3, 0.3, 1.):texture2D(iChannel1, mod((uvr3b - uvrcorr3)/vec2(-ratio, 1.), 1.)));
        }
        else
        {
			...
			if (firstcycle)
                ...
                i = diffb*(colors?vec4(0.3, 1.0, 0.3, 1.):mix(texture2D(iChannel1, mod((uvr3 - uvrcorr3)/vec2(-ratio, 1.), 1.)), getPagebackColor(), bcolorMix));
    ...
    else
    {
        ...
        if (firstcycle)
            i = colors?difftbg*vec4(0.3, 0.3, 1., 1.):texture2D(iChannel1, mod((uvr3bbg - uvrcorr3bg)/vec2(-ratio, 1.), 1.));
        else
